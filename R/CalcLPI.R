...
CalcLPI <- function(Species,
  ...
  for (I in 1:noSpecies) {
    ...
    # Blank matrix of NAs - default value of poplambd
    PopLambda <- matrix(NA, PopIDSize, FinalYear - InitialYear + 1)
    JIndex <- 1
      ...
      if ((length(PopN) >= DATA_LENGTH_MIN) & (AvgTimeBetweenPts < AVG_TIME_BETWEEN_PTS_MAX)) {
        ...
        for (K in StartYear:FinalYear) {
          if ((!is.na(PopN[K - InitialYear + 1])) & (!is.na(PopN[K - InitialYear]))) {
            PopLambda[JIndex, K - InitialYear + 1] <- PopN[K - InitialYear + 1] - PopN[K - InitialYear]
          } else {
            PopLambda[JIndex, K - InitialYear + 1] <- NA
          }
        }
        JIndex <- JIndex + 1
      } else {
    ...
    for (K in 1:EndYear) {
      k <- which(!is.na(PopLambda[, K]))
      if (length(k) > 0) {
        # print(PopLambda)
        # print(sprintf("Year: %d, Selected lambda: %s", K, str(k)))
        # Only get those lambdas that are not 'NA'
        PopLambdaTemp <- PopLambda[k, K]
        # print(sprintf("Year: %d, Selected lambda: %s", K, str(PopLambdaTemp)))
      ...
      # print(sprintf("SpLambda: %f", SpeciesLambda[I, K]))
    }
